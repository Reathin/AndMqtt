...
class DatabaseMessageStore implements MessageStore {
    ...
    public boolean discardArrived(String clientHandle, String id) {
        ...
        try {
            rows = db.delete(ARRIVED_MESSAGE_TABLE_NAME, MqttServiceConstants.MESSAGE_ID + "=? AND "
                    + MqttServiceConstants.CLIENT_HANDLE + "=?", selectionArgs);
        } catch (SQLException e) {
            traceHandler.traceException(TAG, "discardArrived", e);
            throw e;
        }
        if (rows != 1) {
            traceHandler.traceError(TAG, "discardArrived - Error deleting message {" + id
